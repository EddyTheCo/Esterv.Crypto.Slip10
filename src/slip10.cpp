...
/*
 * The function CKDpriv((kpar, cpar), i) → (ki, ci) computes a child extended private key from the parent extended
 * private key
 *
 */
...
Master_key::Master_key(const QByteArray &seed)
    : slip10_key_t(
          QMessageAuthenticationCode::hash(seed, QByteArray("ed25519 seed"), QCryptographicHash::Sha512).constData(),
          64)
{
}
