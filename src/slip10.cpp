...
/*
 * The function CKDpriv((kpar, cpar), i) → (ki, ci) computes a child extended
...
slip10_key_t private_ckd(const slip10_key_t &key, quint32 index) {
  ...
  const auto hmac = QMessageAuthenticationCode::hash(
      data, key.chain_code(), QCryptographicHash::Sha512);
  return slip10_key_t(hmac.constData(), hmac.size());
}

slip10_key_t
Master_key::slip10_key_from_path(const QVector<quint32> &path) const {
...
Master_key::Master_key(const QByteArray &seed)
    : slip10_key_t(QMessageAuthenticationCode::hash(seed,
                                                    QByteArray("ed25519 seed"),
